...
set +e
ssh "$VM_ADDRESS" -t "ls" > /dev/null 2>&1
while [ $? -ne 0 ]; do
    echo "Waiting for VM ..."
    sleep 10
    ssh "$VM_ADDRESS" -t "ls" > /dev/null 2>&1
...
# enable logger
ssh "$VM_ADDRESS" -t "cd "$VM_OBF_DIR" && echo 'Starting build' > build.log"
nohup ssh -t "$VM_ADDRESS" "tail -f "$VM_OBF_DIR"/build.log | ssh 192.168.42.2 'cat >> "$SCRIPT_DIR"/build.log'" >> "$SCRIPT_DIR"/build.log 2>&1 &
LOGGER_PID="$!"

# do work
scp "$SCRIPT_DIR"/oub.zip "$VM_ADDRESS":"$VM_OBF_DIR"
scp "$SCRIPT_DIR"/openjdk.zip "$VM_ADDRESS":"$VM_OBF_DIR"
ssh "$VM_ADDRESS" -t "cd "$VM_OBF_DIR" && "$VM_UNZIP" -q oub.zip >> build.log 2>&1"
ssh "$VM_ADDRESS" -t "cd "$VM_OBF_DIR" && "$VM_UNZIP" -q openjdk.zip >> build.log 2>&1"
ssh "$VM_ADDRESS" -t "cd "$VM_OBF_DIR" && "$VM_START_BUILD" >> build.log 2>&1"

# wait for build to finish
set +e
ssh "$VM_ADDRESS" -t "if [ ! -f "$VM_OBF_DIR"/build_finished.flag ]; then exit 1; else exit 0; fi" > /dev/null 2>&1
while [ $? -ne 0 ]; do
...
scp "$VM_ADDRESS":"$VM_OBF_DIR"/"$BUNDLE_NAME"-image.zip "$SCRIPT_DIR"/dist
scp "$VM_ADDRESS":"$VM_OBF_DIR"/"$BUNDLE_NAME".zip "$SCRIPT_DIR"/dist
...
ssh -t "$VM_ADDRESS" "$VM_SHUTDOWN" >> "$SCRIPT_DIR"/build.log 2>&1 || true
sleep 5
